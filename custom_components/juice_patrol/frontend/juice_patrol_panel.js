...
`;async function Le(e,t){const i=e.shadowRoot.getElementById("jp-chart");if(!i||!t||!t.readings||0===t.readings.length)return;if(!customElements.get("ha-chart-base"))try{await(window.loadCardHelpers?.()),await new Promise(e=>setTimeout(e,100))}catch(e){console.warn("Juice Patrol: loadCardHelpers failed",e)}if(!customElements.get("ha-chart-base"))return void(i.innerHTML='<div class="empty-state">Chart component not available</div>');const a=(t,i)=>{return a=t,n=i,getComputedStyle(e).getPropertyValue(a).trim()||n;var a,n},n=a("--primary-color","#03a9f4"),s=a("--warning-color","#ffa726"),r=a("--error-color","#db4437"),o=a("--success-color","#4caf50");a("--secondary-text-color","#999");const l=a("--secondary-text-color","#999"),c=a("--divider-color","rgba(0,0,0,0.12)"),d=a("--ha-card-background",a("--card-background-color","#fff")),h=a("--primary-text-color","#212121"),p=a("--primary-text-color","#212121"),u=t.readings,m=t.all_readings||u,g=t.prediction,y=t.threshold,_=g.t0??t.first_reading_timestamp,v=m.map(e=>[1e3*e.t,e.v]),b=t.charge_prediction,f="charging"===t.charging_state?.toLowerCase()||"charging"===t.prediction?.status,$=b&&b.estimated_full_timestamp,x=!new Set(["idle","flat","single_level","insufficient_data","insufficient_range","noisy","charging"]).has(g.status);let w=[];if(x&&g.prediction_curve&&g.prediction_curve.length>=2){const e=!f&&"normal"===g.status,t=Date.now();for(const[i,a]of g.prediction_curve){if(!e&&i>t)break;w.push([i,Math.max(0,Math.min(100,a))])}}else if(x&&null!=g.slope_per_day&&null!=g.intercept&&null!=_){const e=e=>{const t=(e-_)/86400;return g.slope_per_day*t+g.intercept},t=u[0].t,i=Date.now()/1e3,a=!f&&"normal"===g.status,n=a?g.estimated_empty_timestamp?Math.min(g.estimated_empty_timestamp,i+31536e3):i+2592e3:i;for(const s of[t,i,...a?[n]:[]])w.push([1e3*s,Math.max(0,Math.min(100,e(s)))])}const S=v[0]?.[0]||Date.now(),E=Date.now();let C;if(f&&b&&null!=b.segment_start_timestamp){const e=b.segment_start_level,i=t.level,a=$?b.slope_per_hour:i>e&&E/1e3-b.segment_start_timestamp>0?(i-e)/((E/1e3-b.segment_start_timestamp)/3600):0;if(a>0){const e=$?1e3*b.estimated_full_timestamp:E+(100-i)/a*36e5;C=e+.1*(e-S)}else C=E+.2*(E-S)}else C=f?E+.2*(E-S):w.length?w[w.length-1][0]:v[v.length-1]?.[0]||E;const A={"1d":864e5,"1w":6048e5,"1m":2592e6,"3m":7776e6,"6m":15552e6,"1y":31536e6};let k,L;const R=e._chartRange||"auto";if("auto"===R)k=S,L=C;else if("all"===R){const e=m.length?1e3*m[0].t:S,t=g.estimated_empty_timestamp?1e3*g.estimated_empty_timestamp:null,i=w.length?w[w.length-1][0]:E;k=Math.min(e,S),L=Math.max(i,t||0,C)}else{const e=A[R]||2592e6;k=E-e;const t=w.length?w[w.length-1][0]:E;L=Math.max(E+e,t)}const T=[];if(t.is_rechargeable&&m.length>=3){const e=[];let t=0;for(let i=1;i<m.length;i++){const a=m[i].v-m[i-1].v;a>1?(-1===t&&e.push({type:"min",idx:i-1,t:m[i-1].t,v:m[i-1].v}),t=1):a<-1&&(1===t&&e.push({type:"max",idx:i-1,t:m[i-1].t,v:m[i-1].v}),t=-1)}for(let t=0;t<e.length-1;t++)if("min"===e[t].type&&"max"===e[t+1].type){e[t+1].v-e[t].v>=10&&T.push([{xAxis:1e3*e[t].t},{xAxis:1e3*e[t+1].t}])}}const D=[],j=[];for(let e=0;e<v.length;e++){const[t,i]=v[e],a=i<=y;if(D.push(a?[t,null]:[t,i]),j.push(a?[t,i]:[t,null]),e>0){const[n,s]=v[e-1];if(s<=y!==a){const e=n+(t-n)*((y-s)/(i-s));D.splice(-1,0,[e,y]),j.splice(-1,0,[e,y])}}}const H=v.length<=50,z=[{name:"Battery Level",type:"line",data:D,smooth:!1,symbol:H?"circle":"none",symbolSize:4,connectNulls:!1,lineStyle:{width:2,color:n},itemStyle:{color:n},areaStyle:{color:n,opacity:.07}},{name:"Battery Level",type:"line",data:j,smooth:!1,symbol:H?"circle":"none",symbolSize:4,connectNulls:!1,lineStyle:{width:2,color:r},itemStyle:{color:r},areaStyle:{color:r,opacity:.1}}];if(T.length>0)for(let e=0;e<T.length;e++){const t=T[e][0].xAxis,i=T[e][1].xAxis;z.push({name:0===e?"Charging":`Charging ${e+1}`,type:"line",data:[[t,100],[i,100]],symbol:"none",lineStyle:{width:0},areaStyle:{color:"rgba(76, 175, 80, 0.18)",origin:0},silent:!0,tooltip:{show:!1}})}w.length>0&&z.push({name:"Discharge prediction",type:"line",data:w,smooth:!1,symbol:"none",lineStyle:{width:2,type:"dashed",color:s},itemStyle:{color:s}});const M=(e,t)=>({show:!0,formatter:e,position:"left",fontSize:10,color:t,distance:6,backgroundColor:"rgba(0,0,0,0.6)",borderRadius:3,padding:[3,6]});if(f&&b&&null!=b.segment_start_timestamp){const e=b.segment_start_timestamp,i=b.segment_start_level,a=Date.now()/1e3,n=t.level,s="history-based"===b.status;let r=$?b.slope_per_hour:null;if((null==r||r<=0)&&n>i){const t=(a-e)/3600;t>0&&(r=(n-i)/t)}if(r>0){const t=(100-n)/r,l=$?b.estimated_full_timestamp:a+3600*t,c=t=>r*((t-e)/3600)+i,d=s?[[1e3*a,n],[1e3*l,100]]:[e,a,l].map(e=>[1e3*e,Math.max(0,Math.min(100,c(e)))]),h=new Date(1e3*l).toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"}),p=d[d.length-1];d[d.length-1]={value:p,symbol:"diamond",symbolSize:6,label:M(`Est. full ${h}`,o)},z.push({name:"Charge prediction",type:"line",data:d,smooth:!1,symbol:"none",lineStyle:{width:2,type:s?"dotted":"dashed",color:o},itemStyle:{color:o}})}}let P=null;if("normal"===g.status&&null!=g.estimated_empty_timestamp&&!f)if(g.prediction_curve&&g.prediction_curve.length>=2)for(let e=1;e<g.prediction_curve.length;e++){const[t,i]=g.prediction_curve[e-1],[a,n]=g.prediction_curve[e];if(i>=y&&n<y){P=t+(i-y)/(i-n)*(a-t),P<=Date.now()&&(P=null);break}}else if(null!=g.slope_per_day&&g.slope_per_day<0&&null!=g.intercept&&null!=_){const e=_+(y-g.intercept)/g.slope_per_day*86400;1e3*e>Date.now()&&(P=1e3*e)}if(w.length>0&&!g.prediction_curve&&null!=g.slope_per_day&&null!=g.intercept&&null!=_){const e=e=>{const t=(e/1e3-_)/86400;return g.slope_per_day*t+g.intercept},t=g.estimated_empty_timestamp?1e3*g.estimated_empty_timestamp:null,i=w[w.length-1][0];P&&P>i-1&&P<(t||1/0)&&w.push([P,Math.max(0,Math.min(100,e(P)))]),t&&t>i+1&&w.push([t,Math.max(0,e(t))])}if(P){const e=new Date(P),t=(P-Date.now())/36e5<=48?e.toLocaleString(void 0,{day:"numeric",month:"short",hour:"numeric",minute:"2-digit"}):e.toLocaleDateString(void 0,{day:"numeric",month:"short",year:"numeric"});z.push({name:"Low battery",type:"line",data:[{value:[P,0],symbol:"none",symbolSize:0},{value:[P,y],symbol:"diamond",symbolSize:6,label:M(`Low ${t}`,r)}],lineStyle:{width:1,type:"dotted",color:r},itemStyle:{color:r},tooltip:{show:!1}})}const F=t.replacement_history||[];if(F.length>0){const e=a("--success-color","#4caf50"),t=F.map(e=>1e3*e).sort((e,t)=>e-t),i=.05*((L||t[t.length-1])-(k||t[0]));for(let a=0;a<F.length;a++){const n=1e3*F[a],s=new Date(n).toLocaleDateString(void 0,{day:"numeric",month:"short"}),r=t.indexOf(n),o=r>0?t[r-1]:null,l=null!=o&&n-o<i?85:100,c=M(`Replaced ${s}`,e);c.position="right",z.push({name:0===a?"Replaced":`Replaced ${a+1}`,type:"line",data:[{value:[n,0],symbol:"none",symbolSize:0},{value:[n,l],symbol:"diamond",symbolSize:6,label:c}],lineStyle:{width:1,type:"dashed",color:e},itemStyle:{color:e},tooltip:{show:!1}})}}const U=t.suspected_replacements||[];if(U.length>0){const e="#ab47bc";for(let t=0;t<U.length;t++){const i=1e3*U[t].timestamp,a=M(`Replaced? ${new Date(i).toLocaleDateString(void 0,{day:"numeric",month:"short"})}`,e);a.position="right",z.push({name:0===t?"Suspected":`Suspected ${t+1}`,type:"line",data:[{value:[i,0],symbol:"none",symbolSize:0},{value:[i,100],symbol:"diamond",symbolSize:6,label:a}],lineStyle:{width:1,type:"dotted",color:e},itemStyle:{color:e},tooltip:{show:!1}})}}const N=e.narrow||i.clientWidth<500,V={xAxis:{type:"time",axisLabel:{color:l,fontSize:N?10:12},axisLine:{lineStyle:{color:c}},splitLine:{lineStyle:{color:c}}},yAxis:{type:"value",min:0,max:100,axisLabel:{formatter:"{value}%",color:l},axisLine:{lineStyle:{color:c}},splitLine:{lineStyle:{color:c}}},tooltip:{trigger:"axis",backgroundColor:d,borderColor:c,textStyle:{color:h},formatter:e=>{if(!e||0===e.length)return"";const t=new Date(e[0].value[0]);let i=`<b>${t.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})+" "+t.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit"})}</b><br>`;const a=new Set;for(const t of e){if(t.seriesName?.startsWith("Replaced")||"Low battery"===t.seriesName)continue;if(null==t.value[1])continue;if(a.has(t.seriesName))continue;a.add(t.seriesName);const e="number"==typeof t.value[1]?t.value[1].toFixed(1)+"%":"—";i+=`${t.marker} ${t.seriesName}: ${e}<br>`}return i}},legend:{bottom:0,left:"center",data:["Battery Level","Discharge prediction"],textStyle:{color:p,fontSize:N?10:12},itemGap:N?6:12,selected:{"Discharge prediction":!f,"Charge prediction":f}},dataZoom:[{type:"inside",xAxisIndex:0,filterMode:"none",startValue:k,endValue:L}],grid:{left:N?4:12,right:N?4:12,top:8,bottom:N?32:28,containLabel:!0}};let O=e._chartEl;O&&i.contains(O)||(O=document.createElement("ha-chart-base"),i.innerHTML="",i.appendChild(O),e._chartEl=O),O.hass=e._hass,O.height=N?"300px":"400px",requestAnimationFrame(()=>{O.data=z,O.options=V})}function Re(e){const t=e._settingsValues?.low_threshold??20;return{icon:{title:"",type:"icon",moveable:!1,showNarrow:!0,template:e=>V`
        <ha-icon
          icon=${me(e)}
