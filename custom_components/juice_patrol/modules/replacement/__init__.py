"""Battery replacement tracking module.

Manages replacement history: marking batteries as replaced, undoing
replacements, and exposing replacement data on each entity.
"""

from __future__ import annotations

import logging
from typing import TYPE_CHECKING, Any

import voluptuous as vol

from homeassistant.exceptions import HomeAssistantError, ServiceValidationError
from homeassistant.helpers import config_validation as cv

from ...const import DOMAIN
from ...discovery import DiscoveredBattery
from .. import JuicePatrolModule, ServiceDefinition, WsCommandDefinition

if TYPE_CHECKING:
    from ...data.store import DeviceData

_LOGGER = logging.getLogger(__name__)


class ReplacementModule(JuicePatrolModule):
    """Tracks battery replacement history."""

    MODULE_ID = "replacement"
    DEPENDENCIES = ()

    async def async_setup(self) -> None:
        pass

    @classmethod
    def ws_command_definitions(cls) -> list[WsCommandDefinition]:
        return [
            WsCommandDefinition(
                command_type="juice_patrol/mark_replaced",
                schema={
                    vol.Required("entity_id"): cv.entity_id,
                    vol.Optional("timestamp"): vol.Coerce(float),
                },
                handler_method="handle_ws_mark_replaced",
            ),
            WsCommandDefinition(
                command_type="juice_patrol/undo_replacement",
                schema={
                    vol.Required("entity_id"): cv.entity_id,
                    vol.Optional("timestamp"): vol.Coerce(float),
                },
                handler_method="handle_ws_undo_replacement",
            ),
        ]

    @classmethod
    def service_definitions(cls) -> list[ServiceDefinition]:
        return [
            ServiceDefinition(
                "mark_replaced",
                "handle_svc_mark_replaced",
                vol.Schema({vol.Required("entity_id"): cv.entity_id}),
            ),
        ]

    def get_entity_data(
        self,
        entity_id: str,
        battery: DiscoveredBattery,
        base_data: dict[str, Any],
        device_data: DeviceData | None,
    ) -> dict[str, Any]:
        return {
            "replacement_history": device_data.replacement_history if device_data else [],
            "last_replaced": device_data.last_replaced if device_data else None,
        }

    async def async_mark_replaced(self, entity_id: str) -> bool:
        if not self.coordinator.store.mark_replaced(entity_id):
            return False
        self.hass.async_create_task(self.coordinator.async_request_refresh())
        return True

    async def async_mark_replaced_at(
        self, entity_id: str, timestamp: float
    ) -> bool:
        if not self.coordinator.store.mark_replaced_at(entity_id, timestamp):
            return False
        self.hass.async_create_task(self.coordinator.async_request_refresh())
        return True

    async def async_undo_replacement(
        self, entity_id: str, *, timestamp: float | None = None
    ) -> bool:
        if timestamp is not None:
            if not self.coordinator.store.remove_replacement(entity_id, timestamp):
                return False
        elif not self.coordinator.store.undo_replacement(entity_id):
            return False
        self.hass.async_create_task(self.coordinator.async_request_refresh())
        return True

    # -- WS handler methods --

    async def handle_ws_mark_replaced(self, hass, connection, msg):
        entity_id = msg["entity_id"]
        timestamp = msg.get("timestamp")
        try:
            if timestamp is not None:
                ok = await self.async_mark_replaced_at(entity_id, timestamp)
            else:
                ok = await self.async_mark_replaced(entity_id)
            if not ok:
                connection.send_error(
                    msg["id"], "not_found",
                    f"Entity {entity_id} not in store",
                )
                return
            connection.send_result(msg["id"], {"ok": True})
        except HomeAssistantError:
            connection.send_error(
                msg["id"], "not_found",
                f"Entity {entity_id} not in store",
            )

    async def handle_ws_undo_replacement(self, hass, connection, msg):
        entity_id = msg["entity_id"]
        timestamp = msg.get("timestamp")
        if await self.async_undo_replacement(entity_id, timestamp=timestamp):
            connection.send_result(msg["id"], {"ok": True})
        else:
            connection.send_error(
                msg["id"], "not_found",
                f"Entity {entity_id} not in store",
            )

    # -- Service handler methods --

    async def handle_svc_mark_replaced(self, call):
        entity_id = call.data["entity_id"]
        if entity_id not in self.coordinator.discovered:
            raise ServiceValidationError(
                translation_domain=DOMAIN,
                translation_key="entity_not_monitored",
                translation_placeholders={"entity_id": entity_id},
            )
        if not await self.async_mark_replaced(entity_id):
            raise HomeAssistantError(
                translation_domain=DOMAIN,
                translation_key="entity_not_in_store",
                translation_placeholders={"entity_id": entity_id},
            )
